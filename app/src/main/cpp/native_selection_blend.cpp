...
Java_com_projectnuke_keplerstudio_bridge_NativePhotoCore_nativeBlendSelectionLayerInPlaceNative(
    ...
    void* localPixels = nullptr;
    void* maskPixels = nullptr;
    if (AndroidBitmap_lockPixels(env, targetBitmap, &targetPixels) != ANDROID_BITMAP_RESULT_SUCCESS || targetPixels == nullptr) return -4;
    if (AndroidBitmap_lockPixels(env, localBitmap, &localPixels) != ANDROID_BITMAP_RESULT_SUCCESS || localPixels == nullptr) {
    ...
    if (AndroidBitmap_lockPixels(env, maskBitmap, &maskPixels) != ANDROID_BITMAP_RESULT_SUCCESS || maskPixels == nullptr) {
        AndroidBitmap_unlockPixels(env, localBitmap);
        AndroidBitmap_unlockPixels(env, targetBitmap);
        return -6;
    }

    auto* target = static_cast<uint8_t*>(targetPixels);
    const auto* local = static_cast<const uint8_t*>(localPixels);
    const auto* mask = static_cast<const uint8_t*>(maskPixels);
